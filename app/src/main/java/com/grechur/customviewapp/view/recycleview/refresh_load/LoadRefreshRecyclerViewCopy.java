...
public class LoadRefreshRecyclerViewCopy extends RefreshRecyclerView {
    // 上拉加载更多的辅助类
    ...
    public LoadRefreshRecyclerViewCopy(Context context) {
        super(context);
    }

    public LoadRefreshRecyclerViewCopy(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public LoadRefreshRecyclerViewCopy(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    private void restoreLoadView() {
        ...
        if (mCurrentLoadStatus == LOAD_STATUS_LOOSEN_LOADING&&this.getAdapter().getItemCount()>2) {
    ...
    public boolean onTouchEvent(MotionEvent e) {
        switch (e.getAction()) {
            case MotionEvent.ACTION_MOVE:
                // 如果是在最底部才处理，否则不需要处理
                Log.e("TAG","canScrollDown"+canScrollDown());
    ...
    private void updateLoadStatus(int distanceY) {
        ...
        } else if (distanceY < mLoadViewHeight&& distanceY>0) {
